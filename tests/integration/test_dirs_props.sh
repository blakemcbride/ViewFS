...
"$VFS" prop set proj:/blake author blake --flow >/dev/null
"$VFS" dir mkdir proj /blake/2024 >/dev/null
"$VFS" prop set proj:/blake/2024 year 2024 >/dev/null
...
"$VFS" prop set "$cid" author jane >/dev/null
"$VFS" prop set "$cid" year 2024 >/dev/null
...
"$VFS" prop set "$bid" author jane >/dev/null   # b now has blake AND jane
"$VFS" dir mkdir proj /both >/dev/null
"$VFS" prop set proj:/both author blake >/dev/null
"$VFS" prop set proj:/both author jane >/dev/null
...
# prop unset removes one value
"$VFS" prop unset "$bid" author jane >/dev/null
...
"$VFS" prop set proj:/empty key nomatch >/dev/null
