/* `viewfs mounts` -- list the ViewFS views currently mounted and where.
 *
...
int cmd_mounts(int argc, char **argv) {
    if (argc > 2 && cli_is_help_request(argv[2])) {
        printf("Usage: viewfs mounts\n"
               "  List the ViewFS views currently mounted and their "
               "mountpoints.\n");
        return 0;
    }
    if (argc != 2) {
        fprintf(stderr, "Usage: viewfs mounts\n");
        return 2;
    }
    ...
    if (rc < 0) {
        fprintf(stderr, "viewfs: cannot read /proc/mounts\n");
        return 1;
    }
    if (c.n == 0) {
